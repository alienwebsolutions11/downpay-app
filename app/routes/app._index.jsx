...
const purchases = await new Promise((resolve, reject) => {
  db.query(
    `SELECT *
     ...
     ORDER BY id DESC`,
    [session.shop],
    (err, results) => (err ? reject(err) : resolve(results.rows))
  );
});
...
export const action = async ({ request }) => {
 ...
 await new Promise((resolve, reject) => {
  db.query(
    ...
    (shop, purchase_option_name, line_item_text, selection_type, products, variants, tags, whole,
    ...
    [
      session.shop, // ✅ YOU MISSED THIS BEFORE
      purchaseName,
      lineItemText,
      selectiontype,
      products,
